...
- **Licence** — the code (`scripts/`, `src/`, the workflows, `site.config.json`) is [GPL-3.0-or-later](LICENSE.txt); the words (everything in `content/`) are [CC BY-SA 4.0](LICENSE-CONTENT.txt)
- **[Third-Party Notices](THIRD_PARTY_NOTICES.md)** — the two typefaces the site carries, under the SIL Open Font License
- **[Trademarks](https://github.com/clementrahula/lukotta/blob/main/TRADEMARKS.txt)** — the Lukotta name, logo, mark and the icons generated from them are under neither licence
